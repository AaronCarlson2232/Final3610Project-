...
runtime = 400*10^-3; % in units of seconds 
...
dutyCycle = 25; %  If this number is too small, the motor won't be able 
         ...
         % to see the impact.
nb.setMotor(2,dutyCycle); % turn on the motor
pause(1); % Wait 1 second to allow the motor to reach a constant speed
val = nb.encoderRead(0); % Initial read to set the counter to zero
pause(runtime);
val = nb.encoderRead(1); % Read again to get counts since last read
nb.setMotor(2,0); % turn off the motor
...
sampleFreq = 1/val; % how many counts did you get over your runtime?
...
maxDutyCycle = 12; % use here the max value you came up with in the last 
                    ...
                    % aliasing).
nb.setMotor(3,dutyCycle);
pause(1);
integral = integral + error * dt; % Update the integral term
derivative = (error - prevError) / dt; % Calculate the derivative term
...
kp = .091;         % proportional gain (applies to the current error)
ki = .350;         % integral gain (applies to the accumulation of past 
%% 
                  % errors over time)
kd = .000001;         % derivative gain (applies to the rate of change of the 
...
while toc < runtime
    ...
    error = 80-rpm; % Difference between our target and current rpm 
    ...
    derivative = /dt; % Rate of change estimate using the difference
    ...
    control = (kp*error)+(ki*integral)+(kd); % Put the error terms and coefficients together
    ...
    nb.setMotor(2, control); % Send the control signal to the motor.
end
