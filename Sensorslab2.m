...
% If you choose to use live plotting (uncomment lines below):
% nb.pinMode('?', 'ainput');
% nb.livePlot('analog', '?');
% maxVal = '?'

% If you choose to use sampling (uncomment lines below):
...
% interp1([min_of_value_to_be_converted max_of_value_to_be_converted], [min_of_desired_range max_of_desired_range], value_to_be_converted);

    % Although in this special case, you could use a more simple 
    % mathematical expression, use interp1 here to convert the ADC output 
    % to an intensity value for the LED (so you know how to use interp1 in 
    % the future when the conversion is not as straight-forward):
...
recordRiseTime = riseTime;  % Record here the riseTime you get for future 
                       % reference
