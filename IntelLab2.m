...
for a = 1:digitCount %iterate through all digits
    for b = 1:trialCount %iterate through all trials
        ...
        Features(a,b,1) = min(y); % Delete rand(1,1) and replace it with 
                                     % the feature you want to extract from
                                     % the x-axis data
        Features(a,b,2) = max (y); % feature for the y-axis data
        Features(a,b,3) =  rms(y); % feature for the z-axis data
...
y= singleLetter[2,:];
z= singleLetter[3,:];
LDAinput(1,1) = min(x); % REPLACE THE RIGHT SIDE OF THESE EQUATIONS WITH
LDAinput(1,2) = max(x); % WHAT YOU ENDED UP USING ABOVE FOR THE
LDAinput(1,3) = rms(z); % TRAINING (SO THAT THE TRAINING AND TESTING USE
                           % THE SAME APPROACH).
